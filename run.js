...
function decodeEntities(s) {
  return String(s)
    ...
    .replace(/&mdash;|&#8212;/gi, '—')
    .replace(/&hellip;|&#8230;/gi, '…')
    // Single angle-quote marks — seen as a decorative "view more" chevron
    // appended to career-page link text (e.g. Filevine's "Executive
    // Assistant › "), not just as actual punctuation. Left undecoded, the
    // literal string "&rsaquo;" flows through as part of the job title and
    // then gets re-escaped for HTML display, showing up on the board as the
    // raw entity name instead of either the arrow or nothing.
    .replace(/&rsaquo;|&#8250;|&#x203a;/gi, '›')
    .replace(/&lsaquo;|&#8249;|&#x2039;/gi, '‹')
...
const RESIDENCY_LOCKED = /\b(u\.?s\.? citizens? only|must be (a )?u\.?s\.? citizen|must reside in the (united states|u\.?s\.?)|u\.?s\.?[- ]based candidates only|no (visa sponsorship|international candidates)|not open to candidates outside the (u\.?s\.?|united states)|EU residents? only|UK residents? only|singapore(an)? citizens? only|malaysian citizens? only|australian citizens? only|must (be|reside) in singapore|must (be|reside) in malaysia|must (be|reside) in australia|must (have|possess) (u\.?s\.?|united states) work authorization|requires? (u\.?s\.?|united states) work authorization|authoriz(?:ed|ation) to work in the (u\.?s\.?|united states) (?:is required|without (?:sponsorship|visa))|must have a green card|w-?2 only|no c2c|no corp-to-corp|physically located (?:in|within) [a-z]+ time ?zone|must be physically (?:located|present|based) in)\b/i;
